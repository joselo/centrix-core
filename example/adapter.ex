defmodule Adapter do
  def build_invoice_xml(invoice_params) do
    case CentrixCore.XmlBuilder.build_invoice(invoice_params[:factura]) do
  ...
  def send_invoice_xml(xml_signed_path, environment \\ 1) do
    case CentrixCore.SriClient.send_document(
  ...
  def auth_invoice(access_key, environment \\ 1) do
    case CentrixCore.SriClient.is_authorized(
           access_key,
           environment
         ) do
      {:ok, %{status: sri_status, response: response}} ->
  ...
  def pdf_invoice_xml(xml_signed_path) do
    xml_parsed = CentrixCore.InvoiceXmlParser.parse_xml(File.read!(xml_signed_path))

    case CentrixCore.InvoicePdfBuilder.build(xml_parsed) do
      {:error, error} -> {:error, error}
      pdf -> {:ok, pdf}
    end
  end
