defmodule BillingCore.XmlRetentionBuilderTest do
  use ExUnit.Case

  alias BillingCore.XmlRetentionBuilder

  describe "build_retention/1" do
    test "build retention and returns the xml and clave_acceso" do
      retencion_params = get_retencion_params()
      clave_acceso_expected = "0405202607110367180400110011000000000330000000114"

      assert {:ok, [xml: xml, clave_acceso: clave_acceso]} =
               XmlRetentionBuilder.build_retention(retencion_params)

      assert clave_acceso == clave_acceso_expected
      assert xml

      expected_xml =
        "test/fixtures/retencion/retencion.xml"
        |> Path.expand()
        |> File.read!()
        |> String.trim()

      assert String.replace(xml, ~r/\s/, "") == String.replace(expected_xml, ~r/\s/, "")
    end

    test "doesn't build the retention and return errors" do
      assert {:error, _error} =
               XmlRetentionBuilder.build_retention(%{})
    end
  end

  def get_retencion_params() do
    info_tributaria_params = %{
      ambiente: 1,
      tipo_emision: 1,
      razon_social: "CARRION JUMBO JOSE AUGUSTO",
      nombre_comercial: "INITMAIN",
      ruc: "1103671804001",
      cod_doc: 7,
      estab: 1,
      pto_emi: 100,
      secuencial: 33,
      dir_matriz: "Ciudadela: DAMMER II Calle: N49C Número: EC-102 Intersección: EL MORLAN",
      clave: %{
        fecha_emision: "2026-05-04",
        tipo_comprobante: 7,
        ruc: "1103671804001",
        ambiente: 1,
        estab: 1,
        pto_emi: 100,
        secuencial: 33,
        codigo: 1,
        tipo_emision: 1
      }
    }

    info_comp_retencion_params = %{
      fecha_emision: "2026-05-04",
      dir_establecimiento: "Ciudadela: DAMMER II Calle: N49C Número: EC-102 Intersección: EL MORLAN",
      obligado_contabilidad: "NO",
      tipo_identificacion_sujeto_retenido: "04",
      parte_rel: "NO",
      razon_social_sujeto_retenido: "Novaux Inc.",
      identificacion_sujeto_retenido: "465219513",
      periodo_fiscal: "05/2026"
    }

    docs_sustento_params = [
      %{
        cod_sustento: "01",
        cod_doc_sustento: "01",
        num_doc_sustento: "001100000000433",
        fecha_emision_doc_sustento: "2026-05-04",
        num_aut_doc_sustento: "0302202001110367180400110011000000000330000000110",
        pago_loc_ext: "01",
        total_sin_impuestos: 100.00,
        importe_total: 112.00,
        impuestos_doc_sustento: [
          %{
            cod_impuesto_doc_sustento: "2",
            codigo_porcentaje: "2",
            base_imponible: 100.00,
            tarifa: 12.00,
            valor_impuesto: 12.00
          }
        ],
        retenciones: [
          %{
            codigo: "1",
            codigo_retencion: "312",
            base_imponible: 100.00,
            porcentaje_retener: 1.75,
            valor_retenido: 1.75
          }
        ],
        pagos: [
          %{
            forma_pago: "01",
            total: 110.25
          }
        ]
      }
    ]

    info_adicional_params = [
      %{
        nombre: "Direccion",
        valor: "East 109 St - 6J Manhattan NY"
      },
      %{
        nombre: "Email",
        valor: "javier@saborpos.com"
      }
    ]

    %{
      info_tributaria: info_tributaria_params,
      info_comp_retencion: info_comp_retencion_params,
      docs_sustento: docs_sustento_params,
      info_adicional: info_adicional_params
    }
  end
end
